...
const gridStagger: Variants = {
  ...
  show: {
    transition: {
      staggerChildren: 0.05, // suave, sin delay por idx en JS
      delayChildren: 0.05,
    },
  },
};
...
const SimpleCard = memo(function SimpleCard({
  ...
  return (
    ...
    >
      {/* glow: blur menor en móvil */}
      <div
        className={`pointer-events-none absolute -top-20 -right-20 h-64 w-64 rounded-full opacity-70 blur-2xl sm:blur-3xl ${a.glow}`}
      />
...
const FeatureSpotlight = memo(function FeatureSpotlight({ item }: { item: FeatureItem }) {
  const a = accentMap(item.accent);

  return (
    <div className="relative overflow-hidden rounded-[24px] border border-white/10 bg-white/[0.035] p-6 sm:p-8">
      <div
        className={`pointer-events-none absolute -top-24 -right-24 h-80 w-80 rounded-full blur-2xl sm:blur-3xl ${a.glow}`}
      />
      ...
      <div className="relative grid gap-6 lg:grid-cols-[1.3fr_0.7fr] lg:items-center">
        ...
        <div className="relative">
          <div className="relative mx-auto grid aspect-square w-full max-w-[300px] place-items-center rounded-[22px] border border-white/10 bg-white/5">
            <div
              className={`pointer-events-none absolute -inset-10 rounded-[26px] blur-2xl sm:blur-3xl ${a.glow}`}
            />
            <div className="pointer-events-none absolute inset-0 rounded-[22px] ring-1 ring-white/10" />
...
const FeatureCard = memo(function FeatureCard({ item }: { item: FeatureItem }) {
  const a = accentMap(item.accent);
  ...
  return (
    <SimpleCard accent={item.accent}>
      ...
      <p className={`mt-3 text-sm leading-6 text-zinc-300 ${clamp2}`}>{item.desc}</p>
...
export default function Features() {
  ...
  const rest = ITEMS.filter((i) => i !== spotlight);
    ...
    <LazyMotion features={domAnimation}>
      <section id="features" className="px-6">
        <div className="mx-auto max-w-6xl py-14 sm:py-20">
          ...
          {/* Spotlight (sin motion: ya es visible arriba, y así ahorras) */}
          <div className="mt-8 sm:mt-10">
            <FeatureSpotlight item={spotlight} />
          </div>

          {/* Grid: 1 animación + stagger interno */}
          <m.div
          ...
          >
            {rest.map((it) => (
              <m.div key={it.title} variants={fadeUp} className="h-full">
                <FeatureCard item={it} />
              </m.div>
            ))}
          </m.div>
